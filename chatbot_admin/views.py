...
from chatbot_admin.services.chatbot_engine import ChatbotEngine
from chatbot_admin.services.tools_registry import ToolsRegistry
from chatbot_admin.serializers import AdminChatRequestSerializer, AdminChatResponseSerializer

logger = logging.getLogger(__name__)

@api_view(["POST"])
@permission_classes([IsAuthenticated])
def platform_admin_chat_view(request):
    """
    Main chat endpoint for platform administrators.
    """
    if request.user.role != "ADMIN":
        return Response(
            {"success": False, "error": "Only platform administrators can access this chatbot."},
    ...
    if audio_file:
        ...
        if transcription:
            ...
            logger.info(f"Admin Whisper Transcription: {message}")
    ...
    try:
        ...
        logger.error(f"Platform Admin Chatbot error: {str(e)}")
        return Response(
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def platform_admin_tools_view(request):
    """
    Returns available tools for the platform admin chatbot.
    """
    return Response({
        "success": True,
